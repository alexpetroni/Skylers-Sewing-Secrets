...
<section class="bg-ivory-50 py-24 sm:py-32">
	<div class="mx-auto max-w-7xl px-6 lg:px-8">
		<div class="mx-auto max-w-2xl lg:text-center">
			<p class="eyebrow">Modules Content Overview</p>
		...
		<div class="mx-auto mt-16 max-w-2xl sm:mt-20 lg:mt-24 lg:max-w-none">
			<dl class="grid max-w-xl grid-cols-1 gap-x-8 gap-y-10 lg:max-w-none lg:grid-cols-2 xl:grid-cols-3">
				{#each modules as module}
					<div class="relative flex flex-col bg-ivory-50 rounded-2xl p-6 shadow-sm ring-1 ring-charcoal-200 hover:shadow-md transition-shadow">
						<dt class="flex items-center gap-x-3 text-base font-semibold leading-7 text-charcoal-900">
						...
						<dd class="mt-4 flex flex-auto flex-col text-sm leading-6 text-charcoal-600">
							<p class="flex-auto text-charcoal-500 mb-3">{module.description}</p>
							<p class="text-xs font-semibold text-charcoal-400">
								{module.videos} videos ({module.minutes} min) {#if module.tutorial_slides > 0}+ {module.tutorial_slides} tutorial slides{/if}
							</p>
						</dd>
					</div>
				{/each}
